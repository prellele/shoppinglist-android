...
public class ShoppinglistDataSource {
	...
	public void addAllToHistory() {
		...
		final String sqlInsertHistory = "INSERT INTO " + DBConstants.TAB_HISTORY_NAME + " ("
				...
				+ DBConstants.COL_HISTORY_QUANTITY + ") SELECT " + DBConstants.COL_SHOPPINGLIST_ID + ", "
				+ replaceUmlautsHistoryPart1 + DBConstants.COL_STORE_NAME + replaceUmlautsHistoryPart2 + ", "
				+ replaceUmlautsHistoryPart1 + DBConstants.COL_PRODUCT_NAME + replaceUmlautsHistoryPart2 + ", "
				+ replaceUmlautsHistoryPart1 + DBConstants.COL_UNIT_NAME + replaceUmlautsHistoryPart2 + ", "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_QUANTITY + " FROM "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " INNER JOIN "
				+ DBConstants.TAB_SHOPPINGLIST_NAME + " on " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_SHOPPINGLIST_ID + " = "
				+ DBConstants.TAB_SHOPPINGLIST_NAME + "." + DBConstants.COL_SHOPPINGLIST_ID + " INNER JOIN "
				+ DBConstants.TAB_STORE_NAME + " on " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_STORE_ID + " = " + DBConstants.TAB_STORE_NAME + "."
				+ DBConstants.COL_STORE_ID + " INNER JOIN " + DBConstants.TAB_PRODUCT_NAME + " on "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID + " = " + DBConstants.TAB_PRODUCT_NAME + "."
				+ DBConstants.COL_PRODUCT_ID + " INNER JOIN " + DBConstants.TAB_UNIT_NAME + " on "
				+ DBConstants.TAB_PRODUCT_NAME + "." + DBConstants.COL_PRODUCT_UNIT_ID + " = "
	...
	/**
	 * checks whether there is a favoriteProductMapping (with combination of given storeId and productId) in this
	 * favorite (given favoriteId)
	 * 
	 * @param favoriteId
	 * @param storeId
	 * @param productId
	 ...
	 */
	public FavoriteProductMapping checkWhetherFavoriteProductMappingExists(final int favoriteId, final int storeId,
			final int productId) {
		this.isDbLockedByThread();
		...
		while ((cursor.getCount() != 0) && cursor.moveToNext()) {
			...
			store.setId(cursor.getInt(cursor.getColumnIndex(DBConstants.COL_FAVORITE_PRODUCT_MAPPING_STORE_ID)));
			favoriteProductMapping.setStore(store);

			final Product product = new Product();
			product.setId(cursor.getInt(cursor.getColumnIndex(DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID)));
			favoriteProductMapping.setProduct(product);

			final Favorite favorite = new Favorite();
			favorite.setId(cursor.getInt(cursor.getColumnIndex(DBConstants.COL_FAVORITE_PRODUCT_MAPPING_FAVORITE_ID)));
	...
	/**
	 * checks whether the product is in use in the shoppinglist_product_mapping or favorite_product_mapping
	 * 
	 * @param productId
	 * @return true when the product is NOT in use and could be deleted, false when it is in use
	 */
	public boolean checkWhetherProductIsNotInUse(final int productId) {
		...
		final Cursor shoppingListProductMappingCursor = this.database.rawQuery(sqlShoppingListProductMapping, null);
		...
		final String sqlFavoriteProductMapping = "SELECT " + DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID
				+ " FROM " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID + " = " + productId;

		final Cursor favoriteProductMappingCursor = this.database.rawQuery(sqlFavoriteProductMapping, null);
	...
	/**
	 * checks whether there is a shoppinglistProductMapping (with combination of given storeId and productId)
	 * 
	 * @param storeId
	 * @param productId
	 * @return ShoppinglistProductMapping shoppinglistProductMapping returns null when there exists no mapping
	 */
	public ShoppinglistProductMapping checkWhetherShoppinglistProductMappingExists(final int storeId,
			final int productId) {
		this.isDbLockedByThread();

		final String sqlQuery = "SELECT * FROM " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " WHERE "
		...
		while ((cursor.getCount() != 0) && cursor.moveToNext()) {
			...
			final Store store = new Store();
			store.setId(cursor.getInt(cursor.getColumnIndex(DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_STORE_ID)));
			shoppinglistProductMapping.setStore(store);

			final Product product = new Product();
			product.setId(cursor.getInt(cursor.getColumnIndex(DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID)));
	...
	/**
	 * checks whether the store is in use in the shoppinglist_product_mapping or favorite_product_mapping
	 * 
	 * @param storeId
	 * @return true when the store is NOT in use and could be deleted, false when it is in use
	 */
	public boolean checkWhetherStoreIsNotInUse(final int storeId) {
		this.isDbLockedByThread();

		boolean isNotInShoppingListProductMappingInUse = true;
		final String sqlShoppingListProductMapping = "SELECT " + DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_STORE_ID
				+ " FROM " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
		...
		boolean isNotInFavoriteProductMappingInUse = true;
		final String sqlFavoriteProductMapping = "SELECT " + DBConstants.COL_FAVORITE_PRODUCT_MAPPING_STORE_ID
				+ " FROM " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
	 ...
	 * 
	 * @param unitId
	 * @return true when the unit is NOT in use and could be deleted, false when it is in use
	 */
	public boolean checkWhetherUnitIsNotInUse(final int unitId) {
		this.isDbLockedByThread();

		boolean isNotInProductInUse = true;
		final String sqlQuery = "SELECT " + DBConstants.COL_PRODUCT_UNIT_ID + " FROM " + DBConstants.TAB_PRODUCT_NAME
				+ " WHERE " + DBConstants.TAB_PRODUCT_NAME + "." + DBConstants.COL_PRODUCT_UNIT_ID + " = " + unitId;
	...
	public void close() {
		...
		if ((this.database != null) && !this.database.isDbLockedByOtherThreads() && this.database.isOpen()) {
	...
	public void createNewShoppinglist() {
		...
		final String sqlMarkShoppinglistFinished = "UPDATE " + DBConstants.TAB_SHOPPINGLIST_NAME + " SET "
				+ DBConstants.COL_SHOPPINGLIST_FINISHED_TIME + " = CURRENT_TIMESTAMP WHERE "
				+ DBConstants.COL_SHOPPINGLIST_ID + " = (SELECT MAX(" + DBConstants.COL_SHOPPINGLIST_ID + ") AS "
	...
	public void deleteAllShoppinglistProductMappings() {
		...
		final Cursor productIdCursor = this.database.rawQuery(sqlNoteProductsIdForFurtherCheck, null);
	...
	/**
	 * deletes a favorite and all the mapping belong to this favorite with given favoriteId
	 * 
	 * @param favoriteId
	 */
	public void deleteFavoriteAndItsMappings(final int favoriteId) {
		...
		// temporary save the product id to delete, for the check, whether it
		// could be deleted in table: product
		final String sqlNoteProductsIdForFurtherCheck = "SELECT " + DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID
				+ " FROM " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_FAVORITE_ID + " = " + favoriteId;
		...
		final String sqlDeleteMappings = "DELETE FROM " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_FAVORITE_ID + " = " + favoriteId;
		...
		final String sqlDeleteFavorite = "DELETE FROM " + DBConstants.TAB_FAVORITE_NAME + " WHERE "
				+ DBConstants.TAB_FAVORITE_NAME + "." + DBConstants.COL_FAVORITE_ID + " = " + favoriteId;
	...
	public void deleteFavoriteProductMapping(final int favoriteProductMappingId) {
		...
		final String sqlNoteProductsIdForFurtherCheck = "SELECT " + DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID
				+ " FROM " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "." + DBConstants.COL_FAVORITE_PRODUCT_MAPPING_ID
	...
	public void deleteProduct(final int productId) {
		...
		final String sqlDeleteProduct = "DELETE FROM " + DBConstants.TAB_PRODUCT_NAME + " WHERE "
				+ DBConstants.TAB_PRODUCT_NAME + "." + DBConstants.COL_PRODUCT_ID + " = " + productId;
	...
	public void deleteProductsFromStoreList(final int storeId) {
		...
		final Cursor productIdsCursor = this.database.rawQuery(sqlNoteProductsIdForFurtherCheck, null);
	...
	public void deleteShoppinglistProductMapping(final int shoppinglistProductMappingId) {
		...
		final String sqlNoteProductsIdForFurtherCheck = "SELECT "
				...
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_ID + " = " + shoppinglistProductMappingId;
	...
	public void deleteStore(final int storeId) {
		...
		final String sqlQuery = "DELETE FROM " + DBConstants.TAB_STORE_NAME + " WHERE " + DBConstants.COL_STORE_ID
				+ " = " + storeId;
	...
	public void deleteUnit(final int unitId) {
		...
		final String sqlQuery = "DELETE FROM " + DBConstants.TAB_UNIT_NAME + " WHERE " + DBConstants.COL_UNIT_ID
				+ " = " + unitId;
	...
	public List<Favorite> getAllFavorites() {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_FAVORITE_ID + ", " + DBConstants.COL_FAVORITE_NAME
				+ " FROM " + DBConstants.TAB_FAVORITE_NAME + " ORDER BY " + DBConstants.COL_FAVORITE_NAME;
	...
	public List<Store> getAllStores() {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_STORE_ID + ", " + DBConstants.COL_STORE_NAME + " FROM "
				+ DBConstants.TAB_STORE_NAME + " ORDER BY " + DBConstants.COL_STORE_NAME;
	...
	public List<Unit> getAllUnits() {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_UNIT_ID + ", " + DBConstants.COL_UNIT_NAME + " FROM "
				+ DBConstants.TAB_UNIT_NAME + " ORDER BY " + DBConstants.COL_UNIT_NAME;
	...
	public int getCheckedProductCountForStore(final int storeId) {
		...
		final String sqlQuery = "SELECT COUNT(" + DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID + ") AS "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID + " FROM "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " WHERE "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_STORE_ID + " = " + storeId + " AND "
		...
		while (checkedProductCountCursor.moveToNext()) {
			checkedProductCount = checkedProductCount
					+ checkedProductCountCursor.getInt(checkedProductCountCursor
							.getColumnIndex(DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID));
		}
	...
	public Favorite getFavoriteByName(String favoriteName) {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_FAVORITE_ID + ", " + DBConstants.COL_FAVORITE_NAME
				+ " FROM " + DBConstants.TAB_FAVORITE_NAME + " WHERE UPPER(" + DBConstants.TAB_FAVORITE_NAME + "."
	 ...
	 * 
	 * @param favoriteId
	 * @return List<FavoriteProductMapping> favoriteProductMappings (should be only one entry)
	 */
	public List<FavoriteProductMapping> getFavoriteProductMappingsByFavoriteId(final int favoriteId) {
		this.isDbLockedByThread();

		String sqlQuery = "SELECT * FROM " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " INNER JOIN "
				+ DBConstants.TAB_FAVORITE_NAME + " on " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_FAVORITE_ID + " = " + DBConstants.TAB_FAVORITE_NAME + "."
				+ DBConstants.COL_FAVORITE_ID + " INNER JOIN " + DBConstants.TAB_STORE_NAME + " on "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_STORE_ID + " = " + DBConstants.TAB_STORE_NAME + "."
				+ DBConstants.COL_STORE_ID + " INNER JOIN " + DBConstants.TAB_PRODUCT_NAME + " on "
				+ DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID + " = " + DBConstants.TAB_PRODUCT_NAME + "."
				+ DBConstants.COL_PRODUCT_ID + " INNER JOIN " + DBConstants.TAB_UNIT_NAME + " on "
				+ DBConstants.TAB_PRODUCT_NAME + "." + DBConstants.COL_PRODUCT_UNIT_ID + " = "
				+ DBConstants.TAB_UNIT_NAME + "." + DBConstants.COL_UNIT_ID + " WHERE " + DBConstants.TAB_FAVORITE_NAME
	...
	public List<History> getHistoryByShoppinglistId(final int shoppinglistId) {
		...
		final String sqlQuery = "SELECT * FROM " + DBConstants.TAB_HISTORY_NAME + " INNER JOIN "
				+ DBConstants.TAB_SHOPPINGLIST_NAME + " ON " + DBConstants.COL_HISTORY_SHOPPINGLIST_ID + " = "
				+ DBConstants.COL_SHOPPINGLIST_ID + " WHERE " + DBConstants.COL_HISTORY_SHOPPINGLIST_ID + " = "
				+ shoppinglistId + " ORDER BY " + DBConstants.COL_HISTORY_PRODUCT;
		...
		while (cursor.moveToNext()) {
			...
			history.setProduct(cursor.getString(cursor.getColumnIndex(DBConstants.COL_HISTORY_PRODUCT)));
			...
			history.setQuantity(cursor.getString(cursor.getColumnIndex(DBConstants.COL_HISTORY_QUANTITY)));

			final Shoppinglist shoppinglist = new Shoppinglist();
			shoppinglist.setId(cursor.getInt(cursor.getColumnIndex(DBConstants.COL_SHOPPINGLIST_ID)));
	...
	public List<Shoppinglist> getHistoryShoppinglists() {
		...
		final String sqlQuery = "SELECT DISTINCT " + DBConstants.COL_SHOPPINGLIST_ID + ", "
				+ DBConstants.COL_SHOPPINGLIST_CREATED_TIME + ", " + DBConstants.COL_SHOPPINGLIST_FINISHED_TIME
				+ " FROM " + DBConstants.TAB_HISTORY_NAME + " INNER JOIN " + DBConstants.TAB_SHOPPINGLIST_NAME + " ON "
	...
	public Product getProductByNameAndUnit(String productName, final int unitId) {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_PRODUCT_ID + ", " + DBConstants.COL_PRODUCT_NAME + ", "
				+ DBConstants.COL_PRODUCT_UNIT_ID + " FROM " + DBConstants.TAB_PRODUCT_NAME + " WHERE UPPER("
				+ DBConstants.TAB_PRODUCT_NAME + "." + DBConstants.COL_PRODUCT_NAME + ") = '"
				+ productName.toUpperCase().trim() + "' AND " + DBConstants.COL_PRODUCT_UNIT_ID + " = " + unitId;
	...
	public int getProductCountForStore(final int storeId) {
		...
		while (productCountCursor.moveToNext()) {
			productCount = productCount
					+ productCountCursor.getInt(productCountCursor
							.getColumnIndex(DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID));
		}
	...
	/**
	 * gets the shoppinglistProductMapping (for the given storeId, if the store is not specified -1 should be given)
	 * 
	 * @param storeId
	...
	public List<ShoppinglistProductMapping> getProductsOnShoppingList(final int storeId) {
		...
		String sqlQuery = "SELECT * FROM " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " INNER JOIN "
				+ DBConstants.TAB_SHOPPINGLIST_NAME + " on " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_SHOPPINGLIST_ID + " = "
				+ DBConstants.TAB_SHOPPINGLIST_NAME + "." + DBConstants.COL_SHOPPINGLIST_ID + " INNER JOIN "
				+ DBConstants.TAB_STORE_NAME + " on " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				...
				+ DBConstants.COL_PRODUCT_ID + " INNER JOIN " + DBConstants.TAB_UNIT_NAME + " on "
				+ DBConstants.TAB_PRODUCT_NAME + "." + DBConstants.COL_PRODUCT_UNIT_ID + " = "
				+ DBConstants.TAB_UNIT_NAME + "." + DBConstants.COL_UNIT_ID + " WHERE "
				+ DBConstants.TAB_SHOPPINGLIST_NAME + "." + DBConstants.COL_SHOPPINGLIST_FINISHED_TIME + " is null";
		if (storeId != -1) {
			sqlQuery += " AND " + DBConstants.TAB_STORE_NAME + "." + DBConstants.COL_STORE_ID + " = " + storeId;
		}
		...
		while (cursor.moveToNext()) {
			...
			shoppinglist.setCreatedTime(Timestamp.valueOf(cursor.getString((cursor
					.getColumnIndex(DBConstants.COL_SHOPPINGLIST_CREATED_TIME)))));
			if (cursor.getString((cursor.getColumnIndex(DBConstants.COL_SHOPPINGLIST_FINISHED_TIME))) != null) {
				shoppinglist.setFinishedTime(Timestamp.valueOf(cursor.getString((cursor
						.getColumnIndex(DBConstants.COL_SHOPPINGLIST_FINISHED_TIME)))));
			}
	...
	public Store getStoreById(final int storeId) {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_STORE_ID + ", " + DBConstants.COL_STORE_NAME + " FROM "
				+ DBConstants.TAB_STORE_NAME + " WHERE " + DBConstants.TAB_STORE_NAME + "." + DBConstants.COL_STORE_ID
				+ " = " + storeId + ";";
	...
	public Store getStoreByName(String storeName) {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_STORE_ID + ", " + DBConstants.COL_STORE_NAME + " FROM "
				+ DBConstants.TAB_STORE_NAME + " WHERE UPPER(" + DBConstants.TAB_STORE_NAME + "."
				+ DBConstants.COL_STORE_NAME + ") = '" + storeName.toUpperCase().trim() + "'";
	...
	/**
	 * Gets all stores from the DB (Table: Store) for the overview "for the overview" means that this are only the
	 * stores which are in use
	 * 
	 * @return List<Store> stores
	 */
	public List<Store> getStoresForOverview() {
		...
		final String sqlQuery = "SELECT DISTINCT " + DBConstants.COL_STORE_ID + ", " + DBConstants.COL_STORE_NAME
				+ " FROM " + DBConstants.TAB_STORE_NAME + " INNER JOIN "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " ON " + DBConstants.TAB_STORE_NAME + "."
				+ DBConstants.COL_STORE_ID + " = " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_STORE_ID + " ORDER BY " + DBConstants.COL_STORE_NAME;
	...
	public Unit getUnitByName(String unitName) {
		...
		final String sqlQuery = "SELECT " + DBConstants.COL_UNIT_ID + ", " + DBConstants.COL_UNIT_NAME + " FROM "
				+ DBConstants.TAB_UNIT_NAME + " WHERE UPPER(" + DBConstants.TAB_UNIT_NAME + "."
				+ DBConstants.COL_UNIT_NAME + ") = '" + unitName.toUpperCase().trim() + "'";
	...
	public short getUserConfigurationViewType() {
		...
		while (cursor.moveToNext()) {
			viewType = cursor.getShort(cursor.getColumnIndex(DBConstants.COL_USER_CONFIGURATION_VIEW_TYPE));
		}
		cursor.close();
	...
	public void markShoppinglistProductMappingAsChecked(final int shoppinglistProductMappingId) {
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " SET "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_CHECKED + " = " + GlobalValues.YES + " WHERE "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_ID + " = " + shoppinglistProductMappingId;
	...
	public void markShoppinglistProductMappingAsUnchecked(final int shoppinglistProductMappingId) {
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " SET "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_CHECKED + " = " + GlobalValues.NO + " WHERE "
				+ DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + "."
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_ID + " = " + shoppinglistProductMappingId;
	...
	public void saveFavorite(String name) {
		...
		final String sqlQuery = "INSERT INTO " + DBConstants.TAB_FAVORITE_NAME + " (" + DBConstants.COL_FAVORITE_NAME
				+ ") VALUES ('" + name.trim() + "')";
	 ...
	 * @param quantity
	 */
	public void saveFavoriteProductMapping(final int favoriteId, final int storeId, final int productId,
		...
		final String sqlQuery = "INSERT INTO " + DBConstants.TAB_FAVORITE_PRODUCT_MAPPING_NAME + " ("
				...
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_STORE_ID + ", "
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_PRODUCT_ID + ", "
				+ DBConstants.COL_FAVORITE_PRODUCT_MAPPING_QUANTITY + ") VALUES (" + favoriteId + ", " + storeId + ", "
				+ productId + ", " + quantity + ")";
		this.database.execSQL(sqlQuery);
	}
	...
	public void saveProduct(String name, final int unitId) {
		...
		final String sqlQuery = "INSERT INTO " + DBConstants.TAB_PRODUCT_NAME + " (" + DBConstants.COL_PRODUCT_NAME
				+ ", " + DBConstants.COL_PRODUCT_UNIT_ID + ") VALUES ('" + name.trim() + "', " + unitId + ")";
		this.database.execSQL(sqlQuery);
	}
	 ...
	 * @param checked
	 */
	public void saveShoppingListProductMapping(final int storeId, final int productId, final String quantity,
			final short checked) {
		this.isDbLockedByThread();

		final String queryToGetShoppinglistId = "SELECT MAX(" + DBConstants.COL_SHOPPINGLIST_ID + ") as "
				+ DBConstants.COL_SHOPPINGLIST_ID + " FROM " + DBConstants.TAB_SHOPPINGLIST_NAME + " WHERE "
		...
		final String sqlQuery = "INSERT INTO " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " ("
				...
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_CHECKED + ") VALUES (" + shoppinglistId + ", " + storeId
	...
	public void saveStore(String name) {
		...
		final String sqlQuery = "INSERT INTO " + DBConstants.TAB_STORE_NAME + " (" + DBConstants.COL_STORE_NAME
				+ ") VALUES ('" + name.trim() + "')";
	...
	public void saveUnit(String name) {
		...
		final String sqlQuery = "INSERT INTO " + DBConstants.TAB_UNIT_NAME + " (" + DBConstants.COL_UNIT_NAME
				+ ") VALUES ('" + name.trim() + "')";
	...
	public void updateFavorite(final Favorite favorite) {
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_FAVORITE_NAME + " SET " + DBConstants.COL_FAVORITE_NAME
				+ " = '" + favorite.getName().trim() + "' WHERE " + DBConstants.TAB_FAVORITE_NAME + "."
	...
	public void updateProduct(final Product product) {
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_PRODUCT_NAME + " SET " + DBConstants.COL_PRODUCT_NAME
				+ " = '" + product.getName().trim() + "' , " + DBConstants.COL_PRODUCT_UNIT_ID + " = "
				+ product.getUnit().getId() + " WHERE " + DBConstants.TAB_PRODUCT_NAME + "."
	 ...
	 * @param quantity
	 */
	public void updateShoppinglistProductMapping(final int shoppinglistProductMappingId, final int storeId,
			final int productId, final String quantity) {
		this.isDbLockedByThread();
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_SHOPPINGLIST_PRODUCT_MAPPING_NAME + " SET "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_STORE_ID + " = " + storeId + ", "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_PRODUCT_ID + " = " + productId + ", "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_QUANTITY + " = " + quantity + " WHERE "
				+ DBConstants.COL_SHOPPINGLIST_PRODUCT_MAPPING_ID + " = " + shoppinglistProductMappingId;
	...
	public void updateStore(final Store store) {
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_STORE_NAME + " SET " + DBConstants.COL_STORE_NAME + " = '"
				+ store.getName().trim() + "' WHERE " + DBConstants.TAB_STORE_NAME + "." + DBConstants.COL_STORE_ID
				+ " = " + store.getId();
	...
	public void updateUnit(final Unit unit) {
		...
		final String sqlQuery = "UPDATE " + DBConstants.TAB_UNIT_NAME + " SET " + DBConstants.COL_UNIT_NAME + " = '"
				+ unit.getName().trim() + "' WHERE " + DBConstants.TAB_UNIT_NAME + "." + DBConstants.COL_UNIT_ID
				+ " = " + unit.getId();
	 ...
	 * 
	 * <p>
	 * Checks whether the DB is locked by a thread (current or other). To avoid deadlock-infinite-loop there's a
	 * counter.
	 * </p>
	 * 
	 */
	private void isDbLockedByThread() {
		...
		while (((this.database != null)
				&& (this.database.isDbLockedByCurrentThread() || this.database.isDbLockedByOtherThreads()) && (counter < 1000))) {
			counter++;
		}
	}
}
